...
Chapters 5--7 developed the theoretical and computational infrastructure for time series causal inference: DynamicDML, Panel DML, HAC inference, and FRED integration. This chapter synthesizes these tools into a \textbf{production-ready application}: estimating how competitor pricing affects insurance/annuity sales.
...
\begin{itemize}
    ...
    \item Provide production deployment guide for real data
\end{itemize}
}
...
\begin{itemize}
    ...
    \item \textbf{Section 8.7}: Production Deployment Guide---configuration for real data
...
\begin{itemize}
    ...
    \item \texttt{full}: Stress-test before production deployment
\end{itemize}
...
model_naive = DynamicDML(n_lags=2, model_y="ridge", model_t="ridge")
...
# For production: Use FREDLoader with API key
# loader = FREDLoader()
...
For production, \textbf{always include macro controls} for interest rate-sensitive products.
}
...
# In production, you'd handle the panel structure appropriately
...
For production, monitor rolling estimates for unexpected shifts.
}
...
This script implements the full analysis workflow for production use.
...
# DML imports
from src.dml import PanelDML, RollingWindowDML, DynamicDML
from src.data import FREDLoader, create_synthetic_fred_data
from src.validation import create_insurance_dgp
...
class CompetitorAnalysisConfig:
    ...
    # Data parameters
    use_synthetic: bool = True  # False for production
    n_periods: int = 120
    n_products: int = 10
...
def load_data(config: CompetitorAnalysisConfig) -> Dict:
    ...
    For production: Replace synthetic generators with real data loaders.
    """
...
% ============================================================================
\section{Production Deployment Guide}\label{sec:deployment_guide}
% ============================================================================
...
# Production configuration for competitor pricing DML
...
assert df["date"].min() >= pd.Timestamp(config.fred_start)
    ...
    \item \textbf{Environment variables}: Set production credentials
    \begin{minted}[fontsize=\small]{bash}
export DATABASE_URL="postgresql://user:pass@host/db"
export FRED_API_KEY="your_api_key"
    \end{minted}

    \item \textbf{Run validation first}: Before production run, validate on synthetic
    \begin{minted}[fontsize=\small]{python}
...
def check_result_quality(result, config):
    """Quality checks for production results."""
    alerts = []
...
\begin{enumerate}
    ...
    \item[(c)] Test for a structural break at $t=60$ using the method from Section~\ref{sec:rolling_window}
...
\begin{enumerate}
    ...
    \item \textbf{Production Pipeline}: End-to-end implementation with configuration management, data quality checks, and monitoring
\end{enumerate}

\textbf{Key Takeaways:}
\begin{itemize}
    \item Always validate on synthetic data before production deployment
