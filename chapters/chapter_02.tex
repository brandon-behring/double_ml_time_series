...
if np.mean(first_stage_T_r2) < 0.1:
    print("\textbackslash{n}⚠️  WARNING: Low first{-stage R² for T. Consider:"})
...
# featurizer=None means we model τ(X) = X\textquotesingle{θ (linear heterogeneity)}
dml_cate = LinearDML(
...
\item
  ...
  variance in both could make result non-significant → \textbf{fragile}
\end{itemize}
...
n), sparse signals * \textbf{Tuning}: Use cross-validation for λ
(penalty parameter)
