...
% ============================================================================
% This chapter develops the core DML methodology:
%   1. The regularization bias problem with ML nuisance estimation
%   2. Neyman orthogonality as the solution
%   3. Cross-fitting for valid inference
%   4. The complete DML algorithm
% ============================================================================

\chapter{Neyman Orthogonality and Double Machine Learning}
\label{ch:neyman_orthogonality_dml}
...
\marginnote[Key Insight]{Orthogonality means first-order bias in nuisance $\hat{\eta}$ doesn't affect $\hat{\tau}$. This is why ML works!}%
...
\begin{definition}[Partially Linear Model]
\label{def:partially_linear_model}
The \textbf{partially linear model} specifies:
\begin{align}
...
where:
\begin{itemize}
\item $Y_i$ is the outcome, $T_i$ is treatment, $X_i$ are confounders
\item $g_0(X)$ is the direct effect of confounders on outcome
\item $m_0(X) = \E[T \mid X]$ is the propensity function
\item $\E[\epsilon_i \mid T_i, X_i] = 0$ (exogeneity)
\item $\E[v_i \mid X_i] = 0$ (treatment residual is mean-zero)
\end{itemize}
\end{definition}
...
Under the model, $\ell_0(X) = \E[Y \mid X] = \tau_0 \cdot m_0(X) + g_0(X)$.

\begin{theorem}[Neyman Orthogonality of DML Score]
\label{thm:dml_orthogonality}
\sidenote{The proof shows that using $\ell_0(X) = \E[Y \mid X]$ instead of $g_0(X)$ achieves orthogonality.}%
...
Now note that $\ell_0(X) = \E[Y \mid X] = \tau_0 m_0(X) + g_0(X)$.
...
by $\E[v \mid X] = 0$ and $\E[\epsilon \mid T, X] = 0$.
...
\[\left. \frac{\partial}{\partial m} \E[\psi_{\text{DML}}] \right|_{\eta = \eta_0} = 0\]

Therefore, $\psi_{\text{DML}}$ is Neyman orthogonal.
\end{proof}
...
\begin{definition}[Cross-Fitting Procedure]
\label{def:cross_fitting}
\marginnote[Why $K$-fold?]{$K=5$ is the default. Larger $K$ uses more data for nuisance estimation but increases computation.}%
The DML cross-fitting procedure:
\begin{enumerate}
\item \textbf{Split data} into $K$ roughly equal-sized folds: $I_1, \ldots, I_K$
\item \textbf{For each fold} $k = 1, \ldots, K$:
  \begin{enumerate}
  \item Estimate nuisance functions on the complement: $\hat{\ell}^{(-k)}$
    and $\hat{m}^{(-k)}$ using $\bigcup_{j \neq k} I_j$
  \item Compute fold-specific estimate $\hat{\tau}_k$ by solving:
    \[\frac{1}{|I_k|} \sum_{i \in I_k} \psi(W_i; \hat{\tau}_k, \hat{\eta}^{(-k)}) = 0\]
  \end{enumerate}
\item \textbf{Aggregate} across folds:
  \[\hat{\tau}_{\text{DML}} = \frac{1}{K} \sum_{k=1}^K \hat{\tau}_k\]
\end{enumerate}
\end{definition}

\margintip[Essential]{Without sample splitting, overfitting in nuisance estimation contaminates $\hat{\tau}$---even with orthogonality!}%
...
\begin{theorem}[Asymptotic Normality of DML]
\label{thm:dml_asymptotic_normality}
\marginnote[Inference]{This theorem guarantees valid confidence intervals even with ML nuisance estimation.}%
...
\[\Sigma = \E\left[\frac{\psi(W_i; \tau_0, \eta_0)^2}{(\E[\partial_\tau \psi(W_i; \tau_0, \eta_0)])^2}\right]\]
\end{theorem}
...
Microsoft's \textbf{EconML} library provides established
...
\sidenote{Random Forest is a good default for DML. Use Gradient Boosting when maximum accuracy is critical.}%
...
\marginwarning[Diagnostic]{$R^2 < 0.1$ is a red flag. Either confounding is weak (good!) or you're missing important confounders (bad!).}%
...
\item
  ...
  differentiable)
\item
  \textbf{Moment bounds}: \(\mathbb{E}[Y_i^4], \mathbb{E}[T_i^4] < \infty\)
\item
...
\textbf{Best for}: High-dimensional linear settings ($p > n$), sparse signals * \textbf{Tuning}: Use cross-validation for $\lambda$
(penalty parameter)
...
datasets ($n > 10{,}000$), image/text confounders *
...
to \texttt{cv=10} for larger datasets ($n > 5{,}000$) *
...
\item
  \textbf{Implementation}: EconML provides established tools with
  automatic inference
\item
...
here, the bias dominates: \(\sqrt{n} \mathbb{E}[\hat{\tau} - \tau_0] = O(n^{-1/2})\), which does not vanish. The estimator is
\textbf{inconsistent}.
