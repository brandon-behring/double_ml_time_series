...
\begin{lstlisting}[language=Julia, caption={MLJ.jl basic usage}]
using MLJ
...
\begin{lstlisting}[language=Julia, caption={CausalInference.jl example}]
...
\begin{lstlisting}[language=Julia, caption={Benchmark setup (Julia)}]
...
CATE inference (batch 10K) & 0.85 & 0.11 & 7.7$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{lstlisting}[language=Python, caption={Python cross-fitting}]
# Python (scikit-learn)
...
\begin{lstlisting}[language=Julia, caption={Julia cross-fitting}]
# Julia (MLJ.jl)
using MLJ, Random
...
\begin{lstlisting}[language=Python, caption={Python DML estimator}]
# Python
class DoubleML:
...
\begin{lstlisting}[language=Julia, caption={Julia DML estimator}]
# Julia
...
\begin{lstlisting}[language=Julia, caption={PyCall integration}]
using PyCall
...
\begin{lstlisting}[language=Python, caption={PythonCall integration}]
# Python
...
\begin{lstlisting}[language=Julia, caption={Native Julia package structure}]
...
export DoubleML, DynamicDML, PanelDML
export fit!, predict_ate, predict_cate

include("estimators/double_ml.jl")
include("estimators/dynamic_dml.jl")
...
\begin{itemize}
    \item \textbf{DynamicDML}: Time-varying treatment effects
...
\subsection{Phase 3: Production (12-18 months)}
...
\begin{lstlisting}[language=Julia, caption={Julia type annotations}]
...
Translate the HAC standard error computation from Chapter 6 (\texttt{src/dml/hac.py}) to Julia. Verify numerical equivalence with the Python implementation on test data.
\end{exercise}
