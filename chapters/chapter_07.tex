...
Chapters 5--6 developed time series and panel DML methods using synthetic data. Production applications require \textbf{real macroeconomic controls}---GDP, inflation, interest rates---that create common confounders across economic units. This chapter integrates our methods with Federal Reserve Economic Data (FRED), the canonical source for U.S.\ macroeconomic time series.
...
The Federal Reserve Bank of St.\ Louis maintains FRED (Federal Reserve Economic Data), containing over 800,000 time series. Our \texttt{FREDLoader} class in \texttt{src/data/fred\_loader.py} provides standardized access.
...
Combining macro controls with DynamicDML:
...
model_macro = DynamicDML(n_lags=2, model_y="ridge", model_t="ridge")
...
# Ridge: shrinks all coefficients (keeps all controls)
model_ridge = DynamicDML(
    n_lags=2,
    model_y=RidgeCV(alphas=[0.1, 1.0, 10.0]),
    model_t=RidgeCV(alphas=[0.1, 1.0, 10.0])
)

# Lasso: performs variable selection (may drop irrelevant macro)
model_lasso = DynamicDML(
    n_lags=2,
...
model_naive = DynamicDML(n_lags=1, model_y="ridge", model_t="ridge")
...
model_rates = DynamicDML(n_lags=1, model_y="ridge", model_t="ridge")
...
model = DynamicDML(n_lags=0, model_y="ridge", model_t="ridge")
...
for sim in range(n_sims):
    ...
    # Naive estimate
    X_naive = np.ones((n, 1))
    model_n = DynamicDML(n_lags=1, model_y="ridge", model_t="ridge")
    res_n = model_n.fit(Y, T, X_naive)
    results_naive.append(res_n.theta)

    # Controlled estimate
    X_ctrl = macro.data.values
    model_c = DynamicDML(n_lags=1, model_y="ridge", model_t="ridge")
