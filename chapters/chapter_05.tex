% Chapter 5: Dynamic Treatment Effects for Time Series
% ============================================================================
...
% - DynamicDML implementation
% ============================================================================

\chapter{Dynamic Treatment Effects for Time Series}
\label{ch:dynamic_treatment_effects}

\section{Introduction}\label{sec:ch05_introduction}

Chapters 1--4 developed DML for cross-sectional data where observations are independent. However, many economic questions involve \textbf{time series data} where observations exhibit temporal dependence. This chapter extends DML to handle autocorrelated treatments, confounders, and outcomes---a critical extension for macroeconomic policy analysis, financial applications, and dynamic pricing.
...
\begin{itemize}
    ...
    \item Apply DynamicDML to estimate dynamic treatment effects
    \item Validate implementation with Monte Carlo simulations
\end{itemize}
}
...
DynamicDML fixes both issues through temporal cross-validation and HAC inference.
}
...
\begin{itemize}
    ...
    \item \textbf{Section 5.4}: DynamicDML Framework---combining the components
...
We implement three strategies in \texttt{src/dml/cross\_fitting.py}:
...
Three commonly used kernels (implemented in \texttt{src/dml/hac.py}):
...
% ============================================================================
\section{The DynamicDML Framework}\label{sec:dynamic_dml}
% ============================================================================

\subsection{Algorithm Overview}

DynamicDML combines time series cross-validation with HAC inference:

\begin{algorithm}[H]
\caption{DynamicDML Estimation}
\label{alg:dynamic_dml}
\begin{algorithmic}[1]
...
Sample Information:
  Observations:          500
  Time periods:          497  (3 lost to lags)
...
We validate DynamicDML through simulation studies examining:
\begin{enumerate}
...
\begin{table}[h]
\centering
\caption{DynamicDML Monte Carlo Results ($n=500$, 200 simulations)}
...
DynamicDML (HAC SE) & 0.02 & 0.12 & 0.11 & 93\% \\
\hline
\end{tabular}
\end{table}
...
\subsection{When to Use DynamicDML}
...
\begin{enumerate}
    ...
    \item \textbf{Dynamic effects}: When lagged treatments affect current outcomes
...
\begin{table}[h]
\centering
\caption{DynamicDML Configuration Guide}
...
\begin{enumerate}
    ...
    \item \textbf{DynamicDML Framework}: Integrates lagged features, temporal CV, and HAC inference into a unified estimation procedure
\end{enumerate}

\textbf{Key Takeaways:}
\begin{itemize}
    \item Standard DML produces correct point estimates but \textbf{invalid SEs} for time series data
    ...
    \item DynamicDML achieves near-nominal coverage in Monte Carlo simulations
\end{itemize}

% ============================================================================
\section{Roadmap to Chapter 6}\label{sec:roadmap}
% ============================================================================
