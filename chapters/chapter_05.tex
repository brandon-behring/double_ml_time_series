...
\keyresult{
The class used in this chapter, \texttt{TemporalPLRDML}, is a \emph{scalar} partially linear estimator: it estimates one treatment effect after controlling for current covariates and lagged treatments, and does not estimate period-specific $\theta_t$ effects. Recursive dynamic g-estimation with period-specific blips is implemented separately as \texttt{DynamicGEstimationDML} in the next chapter (\cref{sec:ch06_dynamic_g}).
}
...
\begin{remark}[Companion v1.1.0]
As of companion release 1.1.0, the temporal-validation infrastructure in
this chapter --- cross-validation splitters, HAC/Newey--West inference, and
stationarity diagnostics --- is consumed from the \texttt{temporalcv}
library rather than maintained inside the companion. Each migration was
gated on golden snapshots of the estimators' numerical output; the one
deliberate behavioral change (the forward-only purged walk replacing a
bidirectional purged K-fold) is a leakage \emph{correction}, documented
where it appears below.
\end{remark}

Three strategies are available (implemented upstream in \texttt{temporalcv}; the companion's estimators select among them via \texttt{cv\_strategy}):
...
\begin{definition}[Purged Walk-Forward CV \cite{dePrado2018}]
\label{def:purged_cv}
For data with overlapping labels or leakage concerns, a forward-only walk
with purging:
\begin{enumerate}
    \item Walk test windows forward through time, training only on
          observations \emph{before} each test window
    \item \textbf{Purge} the last $g$ training observations adjacent to the
          test boundary (the \texttt{purge\_gap}), so labels that overlap
          the test window never enter training
\end{enumerate}
\end{definition}

\begin{remark}
De Prado's original formulation is a \emph{bidirectional} purged K-fold
(training on both sides of each test fold, with a post-test embargo). For
nuisance estimation in temporal DML that bidirectionality is itself
lookahead: an earlier revision of this companion's \texttt{purged\_cv}
strategy trained nuisances on observations after the test window, and was
replaced by the forward-only walk above.
\end{remark}
...
Three commonly used kernels (implemented upstream in \texttt{temporalcv}):
...
The rule-of-thumb bandwidth for the Bartlett kernel, as implemented in
\texttt{temporalcv}:
\begin{equation}
B^* = \left\lfloor n^{1/3} \right\rfloor
\end{equation}

For $n=500$: $B^* = 7$ lags. Constant factors vary across texts and
implementations (Newey \& West's (1994) own rule of thumb is
$4(n/100)^{2/9}$); the growth rate $n^{1/3}$ is what matters for the
Bartlett kernel's consistency.
\end{definition}
...
\begin{remark}
The explicit \texttt{long\_run\_variance} / \texttt{variance} /
\texttt{se} split exists precisely because conflating $\Omega$ with
$\Omega/n$ produces silent $\sqrt{n}$-scale errors in downstream standard
errors; an earlier revision of this companion's estimator divided by $n$
twice at exactly this seam.
\end{remark}
...
\begin{remark}
The HAC bandwidth of 7 was automatically selected by the $\lfloor n_{\text{used}}^{1/3} \rfloor$ rule implemented in \texttt{temporalcv}: for $n_{\text{used}}=410$, $\lfloor 410^{1/3} \rfloor = \lfloor 7.43 \rfloor = 7$.
\end{remark}
...
\begin{enumerate}
    \item[(a)] Calculate the rule-of-thumb bandwidth $B^* = \lfloor n^{1/3} \rfloor$
...
\begin{enumerate}
    \item[(a)] $B^* = \lfloor 1000^{1/3} \rfloor = \lfloor 10 \rfloor = 10$
