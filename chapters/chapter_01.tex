...
\marginnote[Key Idea]{Each unit has two potential outcomes: $Y(0)$ and $Y(1)$. Only one is ever observed.}%
...
\marginwarning[Critical]{This is \emph{not} a statistical problem---no amount of data can solve it!}%
...
For unit $i$:
\begin{itemize}
\item If $T_i = 1$, we observe $Y_i(1)$ but not $Y_i(0)$
\item If $T_i = 0$, we observe $Y_i(0)$ but not $Y_i(1)$
\end{itemize}
...
\label{def:ate}
\marginnote[ATE]{The main estimand in DML. We give up on individual $\tau_i$ and estimate the population average.}%
...
\begin{itemize}
\item $\E[Y_i(1) \mid T_i = 1] = \E[Y_i(1)]$: Treated units are representative of the population
\item $\E[Y_i(0) \mid T_i = 0] = \E[Y_i(0)]$: Control units are representative of the population
\end{itemize}
...
\marginnote[Untestable]{This assumption cannot be directly verified---it involves counterfactuals. Domain knowledge is essential.}%
...
\margintip[Testable]{Unlike unconfoundedness, overlap \emph{can} be checked by examining propensity scores.}%
...
\begin{itemize}
\item $Y_1(1) = 245$: Observed (this actually happened)
\item $Y_1(0) = ?$: Counterfactual (what would have happened if competitors had low prices)
\end{itemize}
...
For each patient $i$:
\begin{itemize}
\item $Y_i(1)$: Change in HbA1c if given new medication
\item $Y_i(0)$: Change in HbA1c if given standard medication
\end{itemize}
...
primarily to:
\begin{itemize}
\item Younger patients (better adherence)
\item Patients with higher baseline HbA1c (more room for improvement)
\item Patients with fewer comorbidities (lower risk of side effects)
\end{itemize}

Now:
\begin{itemize}
\item $T_i$ is related to age, baseline HbA1c, comorbidities
\item These variables also affect $Y_i$ (younger patients may improve more regardless of medication)
\end{itemize}
...
\begin{itemize}
\item \textbf{True medication effect}: $\E[Y_i(1) - Y_i(0)]$
\item \textbf{Patient selection}: Treated patients are younger, healthier, more likely to improve anyway
\end{itemize}
...
\textbf{When plausible}:
\begin{itemize}
\item If doctors prescribe based solely on observable characteristics (age, baseline HbA1c, comorbidities, etc.)
\item All relevant patient features are recorded in electronic health records
\end{itemize}

\textbf{When violated}:
\begin{itemize}
\item Doctors have private information not in the EHR (patient motivation, family support, subtle clinical signs)
\item Patients self-select into treatment based on unobservable factors (fear of side effects, personal preferences)
\end{itemize}
...
\textbf{Practical solution}: Either:
\begin{enumerate}
\item \textbf{Restrict population}: Estimate ATE only for ages 18-75 (where overlap holds)
\item \textbf{Collect more data}: Find hospitals that do prescribe to elderly patients
\item \textbf{Randomized trial}: If effect on elderly is crucial, conduct RCT
\end{enumerate}
...
\textbf{Connection to insurance pricing}: Same framework, different domain:
\begin{itemize}
\item Healthcare: Doctors select patients for treatment based on characteristics
\item Insurance: Competitors set prices based on market conditions
\item Both: Need to condition on confounders to identify causal effects
\end{itemize}
...
\begin{itemize}
\item $e(x) > 0$: Some units with $X = x$ receive treatment $\to$ Can estimate $\E[Y(1) \mid X = x]$
\item $e(x) < 1$: Some units with $X = x$ receive control $\to$ Can estimate $\E[Y(0) \mid X = x]$
\item Both required to estimate $\E[Y(1) \mid X = x] - \E[Y(0) \mid X = x]$
\end{itemize}
...
\begin{enumerate}
\item \textbf{Trimming}: Drop units with $e(x)$ close to 0 or 1 (changes estimand to trimmed population)
\item \textbf{Weighting}: Propensity score weighting (upweight rare treatment assignments)
\item \textbf{Regularization}: Shrink extreme propensity score weights
\end{enumerate}
...
\label{thm:fwl}
\marginnote[Foundation]{FWL is the theoretical foundation for DML. The ``double'' refers to partialling out from both $Y$ and $T$.}%
...
variables with inner product $\langle Y, Z \rangle = \E[YZ]$.
...
where:
\begin{itemize}
\item $P_X$: Projection operator onto the subspace spanned by $X$
\item $\E[T \mid X]$: Component of $T$ in the $X$-subspace
\item $\tilde{T}$: Component of $T$ orthogonal to $X$
\end{itemize}

\textbf{Orthogonality}: By definition of projection:

\[\langle \tilde{T}, h(X) \rangle = \E[\tilde{T} \cdot h(X)] = 0 \quad \text{for all} \quad h\]
...
\textbf{Linear FWL fails}:
\begin{enumerate}
\item Regress $T$ on $X$ (linear regression) $\to$ Misspecified, underestimates $\E[T \mid X]$
\item Regress $Y$ on $X$ (linear regression) $\to$ Misspecified, underestimates $\E[Y \mid X]$
\item Residuals $\tilde{T}, \tilde{Y}$ still contain confounding from $X$
\item Final estimate $\hat{\beta}_1 \neq 2$ is biased
\end{enumerate}

\textbf{Machine learning solution}:
\begin{itemize}
\item Use \textbf{random forests} or \textbf{boosting} to estimate $\E[T \mid X]$ nonparametrically
\item Use \textbf{neural networks} or \textbf{splines} to estimate $\E[Y \mid X]$ flexibly
\item Residuals properly remove nonlinear confounding
\item Final estimate converges to $\beta_1 = 2$
\end{itemize}
...
\begin{itemize}
\item $p$ confounders where $p$ is large (hundreds or thousands)
\item $p$ may even exceed $n$ (more variables than observations)
\end{itemize}

\textbf{Examples}:
\begin{itemize}
\item \textbf{Healthcare}: Electronic health records with thousands of diagnosis codes, lab values, medications
\item \textbf{Marketing}: User demographics, browsing history, click patterns, device info
\item \textbf{Insurance}: Competitor product features, market conditions, economic indicators
\end{itemize}

\textbf{Linear regression breaks down}:
\begin{itemize}
\item $p > n$: Regression is undefined (underdetermined system)
\item $p \approx n$: Regression is highly unstable (overfitting)
\item Large $p$: Need regularization (Lasso, Ridge) $\to$ Introduces bias
\end{itemize}

\textbf{Machine learning handles high dimensions}:
\begin{itemize}
\item \textbf{Random forests}: Split on important variables, ignore noise variables
\item \textbf{Gradient boosting}: Sequentially add weak learners focusing on residuals
\item \textbf{Neural networks}: Learn low-dimensional representations
\item \textbf{Lasso regression}: Automatic variable selection with $\ell_1$ penalty
\end{itemize}

\subsection{The Regularization Bias
Problem}\label{subsec:ch01_regularization_bias}
\marginwarning[Key Problem]{This is why naive ML fails for causal inference. Chapter 2 shows how DML solves it.}%
...
\textbf{Naive FWL with Lasso}:
\begin{enumerate}
\item Estimate $\hat{m}_T(X)$ with Lasso $\to$ biased toward zero
\item Compute $\tilde{T}_i = T_i - \hat{m}_T(X_i)$ $\to$ Residuals \textbf{not} mean-zero
\item Estimate $\hat{\beta}_1$ $\to$ Biased due to regularization bias in step 1
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Sample splitting}: Use different data for estimating $\hat{m}(X)$ and $\hat{\beta}_1$
\item \textbf{Cross-fitting}: Repeat with role reversal and average
\item \textbf{Result}: Regularization bias cancels out, $\hat{\beta}_1 \to \beta_1$
\end{enumerate}
...
\textbf{Linear FWL}:
\begin{itemize}
\item Fast: $O(np^2)$ for $n$ observations, $p$ confounders
\item Scales to $n = 10^6$, $p = 10^3$
\item No hyperparameter tuning needed
\end{itemize}

\textbf{Double ML with Random Forests}:
\begin{itemize}
\item Moderate: $O(n \log n \cdot p \cdot \text{trees})$
\item Requires hyperparameter tuning (max depth, min samples per leaf)
\item Parallelizes well (n\_jobs=48 on 64-core system)
\item Scales to $n = 10^6$, $p = 10^4$
\end{itemize}

\textbf{Double ML with Neural Networks}:
\begin{itemize}
\item Slow: Depends on architecture and optimization
\item Requires careful hyperparameter tuning (layers, width, learning rate, regularization)
\item Benefits from GPU acceleration
\item Best for very large $n$ ($>10^6$) and complex nonlinearity
\end{itemize}

\textbf{Practical workflow}:
\begin{enumerate}
\item Start with linear FWL (fast baseline)
\item Try Random Forest DML (good default for nonlinearity)
\item Use neural networks only if RF insufficient and computational budget allows
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Potential Outcomes Framework}: Defines causal effects through
  counterfactuals $Y_i(1), Y_i(0)$
\item \textbf{Fundamental Problem}: Individual effects
  $\tau_i = Y_i(1) - Y_i(0)$ are never observed
\item \textbf{Average Treatment Effect}: Population-level effect
  $\text{ATE} = \E[Y_i(1) - Y_i(0)]$ is identifiable
\item \textbf{Identification}: Under unconfoundedness and overlap, ATE can
  be expressed using observed data
\item \textbf{FWL Theorem}: Partialling out confounders $X$ from $T$ and
...
\begin{enumerate}
\item Residualizing treatment on confounders: $\tilde{T} = T - \E[T \mid X]$
\item Residualizing outcome on confounders: $\tilde{Y} = Y - \E[Y \mid X]$
\item Regressing orthogonalized outcome on orthogonalized treatment
\end{enumerate}

This partialling-out interpretation generalizes beyond linear
regression:
\begin{itemize}
\item \textbf{Classical econometrics}: Use linear regression for
$\E[T \mid X]$ and $\E[Y \mid X]$
\item \textbf{Modern ML}: Use random forests, boosting, or neural networks for flexible approximation
\end{itemize}
...
\textbf{The DML Algorithm}: The complete procedure with:
\begin{enumerate}
\item \textbf{Sample splitting}: Divide data into $K$ folds
\item \textbf{Cross-fitting}: Estimate nuisance parameters on one fold, treatment effect on another
\item \textbf{Aggregation}: Average across folds
\end{enumerate}

\textbf{Theoretical guarantees}: Under high-level conditions:
\begin{itemize}
\item $\sqrt{n}$-consistency and asymptotic normality
\item Valid confidence intervals using cross-fit standard errors
\item Robustness to slow convergence of ML estimators
\end{itemize}

\textbf{Python implementation}: Working code using EconML with:
\begin{itemize}
\item Random Forest nuisance estimation
\item Inference with confidence intervals
\item Comparison to naive approaches
\end{itemize}
...
\textbf{Solution}: For week $i$:
\begin{itemize}
\item $Y_i(1)$: Number of annuities sold in week $i$ if competitors had high prices
\item $Y_i(0)$: Number of annuities sold in week $i$ if competitors had low prices
\end{itemize}
...
sales. Plausible if:
\begin{itemize}
\item Customers are completely insensitive to competitor prices (no price shopping)
\item Our product is perfectly differentiated (no substitution)
\item Competitor prices don't signal market conditions
\end{itemize}
...
is logically impossible. Collecting more data gives us:
\begin{itemize}
\item More weeks with $T_i = 1$ (high competitor prices)
\item More weeks with $T_i = 0$ (low competitor prices)
\end{itemize}

But for any specific week $i$, we still only observe one potential
outcome. To compute $\tau_i = Y_i(1) - Y_i(0)$, we would need to:
\begin{enumerate}
\item \textbf{Time travel}: Observe week $i$ twice with different competitor prices, or
\item \textbf{Parallel universes}: Observe both treatments simultaneously in the same week
\end{enumerate}
...
\begin{itemize}
\item $T_i$ (competitor pricing) is affected by unobserved confidence
\item $Y_i(0), Y_i(1)$ (our sales under both treatments) are also affected by unobserved confidence
\item Conditioning on observed $X_i$ (VIX, sentiment, treasury rates) is insufficient
\end{itemize}
...
$w_i = 1/(1-e(X_i))$ for controls. When $e(X_i)$ varies across units:
\begin{itemize}
\item Units with $e(X_i) \approx 1$: Low weights (already likely to be treated)
\item Units with $e(X_i) \approx 0$: High weights (unlikely to be treated but got treated anyway)
\end{itemize}

High variance in weights $\to$ Effective sample size $<$ actual sample size.
...
\begin{itemize}
\item \textbf{Near-violations}: If some $e(X_i) \approx 0.01$, weights can be 100+, drastically reducing $n_{\text{eff}}$
\item \textbf{Precision loss}: Effective 213/250 means we lost 37 units worth of information (15\%)
\item \textbf{Variance inflation}: Standard errors increase by $\sqrt{250/213} \approx 1.08$
\end{itemize}

\textbf{Remedies}:
\begin{enumerate}
\item \textbf{Trimming}: Drop units with $e(X_i) < 0.05$ or $e(X_i) > 0.95$
\item \textbf{Weight truncation}: Cap weights at percentiles (e.g., 99th)
\item \textbf{Doubly robust estimation}: Combine weighting with outcome modeling
\end{enumerate}

% References handled by main.tex bibliography
