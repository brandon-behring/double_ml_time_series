...
overlap: * Some covariate values are only observed in treated group →
Cannot estimate \(\mathbb{E}[Y(0) | X = x\){]} * Some covariate values
are only observed in control group → Cannot estimate
\(\mathbb{E}[Y(1) | X = x\){]}
...
is positive (high competitor prices → more sales). But:
...
\item
  ...
  \textgreater{} 25 → No treated units with high VIX → Cannot estimate
...
\begin{itemize}
\item
  For \(\text{age} > 75\): \(e(x) = 0\) → No treated patients → Cannot
...
receive treatment → Can estimate \(\mathbb{E}[Y(1) | X = x\){]} *
\(e(x) < 1\): Some units with \(X = x\) receive control → Can estimate
...
Now: * For \(\text{VIX} < 20\): \(e(x) = 1\) → All weeks have high
competitor prices → Never observe \(Y_i(0)\) * For
\(\text{VIX} \geq 20\): \(e(x) = 0\) → All weeks have low competitor
prices → Never observe \(Y_i(1)\)
...
\begin{itemize}
\item
  \(e(x) = 0.02\): Only 2\% of units with \(X = x\) are treated → Huge
  variance in \(\hat{\mathbb{E}}[Y(1) | X = x\){]}
\item
  \(e(x) = 0.98\): Only 2\% of units with \(X = x\) are controls → Huge
...
\begin{itemize}
\item $\Var(\tilde{T}_i)$ is small → Less ``identifying variation''
\item Standard errors on $\hat{\beta}_1$ are large → Imprecise estimates
...
confounding → Large identifying variation *
\(R^2_{T \sim X} \approx 1\): Strong confounding → Small identifying
variation → Imprecise estimates
...
→ Misspecified, underestimates \(\mathbb{E}[T|X\){]} 2. Regress \(Y\) on
\(X\) (linear regression) → Misspecified, underestimates
...
Problem}\label{_the_regularization_bias_problem}
...
\textbf{Naive FWL with Lasso}: 1. Estimate \(\hat{m}_T(X)\) with Lasso →
biased toward zero 2. Compute \(\tilde{T}_i = T_i - \hat{m}_T(X_i)\) →
Residuals \textbf{not} mean-zero 3. Estimate \(\hat{\beta}_1\) → Biased
...
\textbf{Rule of thumb}: \(p > 20\) or suspected nonlinearity → Try
Double ML
...
\section{Summary}\label{_summary}
...
\section{Exercises}\label{_exercises}
...
→ Competitors raise prices (high demand expected) * Low survey
confidence → Competitors lower prices (low demand expected)

This survey also affects our sales: * High confidence → More consumers
buy (including from us) * Low confidence → Fewer consumers buy
...
\(T\) on \(X\) (linear) cannot capture \(\mathbb{E}[T|X\) = X\_1\^{}2 +
X\_2{]} 2. Regressing \(Y\) on \(X\) (linear) cannot capture
\(\mathbb{E}[Y|X\) = 2\textbackslash mathbb\{E\}{[}T\textbar X{]} +
X\_1\^{}2 + X\_2\^{}3{]} 3. Residuals \(\tilde{T}, \tilde{Y\){]} still
...
High variance in weights → Effective sample size \textless{} actual
sample size.
